# Overview
-  Links to relevant summaries:
  -  Campus Lagoon restoration overview: https://ccber.ucsb.edu/restoration-area-campus-lagoon
  -  Restoration Register newsletter coverage:
     - May 2023: https://escholarship.org/content/qt2352w6kx/qt2352w6kx.pdf
     - September 2024 Restoration Register: https://escholarship.org/content/qt6k11m3gb/qt6k11m3gb_noSplash_02172bc986189bc884e1df74cd0f0419.pdf
  -  Geography news story: https://legacy.geog.ucsb.edu/controlled-burn-on-lagoon-island-targets-rip-gut-grass/


Prescribed burns have been carried out periodically since 2009: 

- 2009 Burn Plot
- 2011 Burn Plot
- 2012 Burn Plot
- 2014 Burn Plot
- 2016 Burn Plot
- 2018 Burn Plot
- 2020 Burn Plot
- 2022 Burn Plot
- 2024 two separate burns
- 2026 Burn Plot


## Sampling design

Note that the spacing is such that sometimes pairs of quadrats from different transects are closer together than pairs of quadrats within the same transect.

Temporal coverage: annual (late spring/early summer?); some start in 2008, skip 2013 and 2014

# TODO: create csv with Campus Lagoon transect metadata

"Campus Lagoon" really represents smaller management areas:
-  Campus Point (7 acres): 4 transects
    -  CP-1, CP-2, CP-3, CP-4 (**last two seem to be missing from Cheadle Center transect web map**). CP-1 and CP-2 monitored since 2010.  
-  Lagoon Island (16 acres): 8 transects
    -  LI-1 to LI-8
        - note that LI-2 largely covers the 2012 burn plots  
-  East Depression:
-  
-  West Depression
-  San Nicholas Wetland
-  Manzanita Village lope


# Data

- At least some data are in Box>CCBER_Data>Campus_Lagoon>Vegetation_Monitoring>Lagoon Island and Campus Point
- Original veg survey data are collected via Survey123: "CCBER VegMonitoring QuadratTransects" (but seems like those data start in 2021?)
- For focal species: compare occurrences within coastal zone (2 km?) in Southern California, e.g., using iNaturalist data


